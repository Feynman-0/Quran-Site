...
const steps = [
  { n: "01", title: "Book a Free Trial Class", desc: "Fill out the form to schedule your free online Quran class. Our team confirms your slot via WhatsApp.", cta: "Book Trial", action: "form" },
  { n: "02", title: "Take Your Free Lesson", desc: "Attend a free Quran lesson with a qualified teacher at your scheduled time — no payment required.", cta: "Chat With Us", action: "chat" },
  { n: "03", title: "Enroll in Full Course", desc: "After your demo class, choose a monthly plan, register, and unlock the full Online Quran Course.", cta: "Enroll Now", action: "form" },
];
          ...
          {steps.map((s, i) => (
            <div key={s.n} className="relative group">
              <div className="relative bg-card border border-border rounded-t-[180px] rounded-b-2xl p-10 pt-16 text-center shadow-soft hover:shadow-emerald hover:-translate-y-2 transition-all duration-500">
                ...
                <button
                  onClick={() => s.action === "chat" ? openTawkChat() : openModal()}
                  className="inline-block text-xs font-bold tracking-[0.2em] uppercase text-gold border-b-2 border-gold pb-1 hover:text-primary hover:border-primary transition-colors cursor-pointer"
                >
                  {s.cta} →
                </button>
              </div>
